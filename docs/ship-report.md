...
Mock/fallback: GPT-4o, Whisper, and ElevenLabs return honest missing-key/provider errors until `.env` has valid keys. Screenshot capture returns an honest unsupported result. Wake word is disabled unless Porcupine key/binding/custom keyword setup exists.
